...
| `p2_reached_projects`, `p2_reached_mw` | n, MW | projects (any sheet, public report) whose Phase II / Facilities Study column is COMPLETE — they received study results at this node | yes |
| `p2_withdrawn_projects`, `p2_withdrawn_mw` | n, MW | of those, the ones that then withdrew (`p2_withdrawn_mw` == `wd_post_phase2_mw`) | yes — with results in hand, not a cause |
...
After `caiso-siting layers screen`: `williamson_act` (Prime / Nonprime / Nonrenewal / FSZ / Mixed / enrolled / blank — DOC 2025 layer, by APN then by polygon) and booleans `excl_base_solar`, `excl_protected_solar`, `excl_technoeconomic_solar`, `tribal_land`, `critical_habitat` (parcel centroid inside a CEC screen polygon). Presence only — the CEC screen layers carry no attributes. No owner names — this project does not read them, as a matter of policy rather than legal obligation.
