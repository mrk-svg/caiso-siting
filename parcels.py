...
def run(name: str, lat: float, lon: float, km: float) -> None:
    ...
    for county in COUNTIES_FOR_POINT:
        for kind, (url, fields, label) in LAYERS.get(county, {}).items():
            try:
                ...
                print(f"  {county}/{kind}: FAILED ({e})", file=sys.stderr); continue
            print(f"  {county}/{kind}: {len(feats)} features within {km} km")
            if kind == "parcels":
                for f in feats:
                    a = f["attributes"]; rings = f.get("geometry", {}).get("rings", [])
            ...
            if p.lat is not None:
                for attrs, rings, label in feats:
                    if any(point_in_ring(p.lat, p.lon, ring) for ring in rings):
                        hit = str(attrs.get(label) or attrs.get("Comb_Zn") or attrs.get("GP") or "").strip(); break
            vals.append(hit)
        if len(df):
            df[col] = vals
    ...
    for (county, kind), feats in overlays.items():
        if kind != "parcels" and not len(df):
            codes = pd.Series([str(a.get(l) or a.get("Comb_Zn") or "").strip() for a, _, l in feats]).value_counts()
...
def main() -> None:
    ...
    ap.add_argument("--lat", type=float); ap.add_argument("--lon", type=float); ap.add_argument("--name")
