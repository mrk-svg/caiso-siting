...
def load_sheet(path: Path, sheet: str, status_label: str) -> pd.DataFrame:
    ...
    df = df[df["project_name"].notna()]  # kill footer/blank rows
...
def main() -> None:
    ...
    ap.add_argument("--file", default="data/publicqueuereport.xlsx")
    ...
    county = by_county(df);            county.to_csv(out / "by_county.csv")
    poi = by_poi(df);                  poi.to_csv(out / "by_poi.csv", index=False)
    clus = by_cluster(df);             clus.to_csv(out / "by_cluster.csv")
    wby = withdrawals_by_year(df);     wby.to_csv(out / "withdrawals_by_year.csv")
    deliv = deliverability(df);        deliv.to_csv(out / "deliverability.csv")
